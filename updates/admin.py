...
async def knowledge_base_stats():
    """知识库统计信息"""
    try:
        from app.knowledge_db import stats as kb_stats
        return kb_stats()
    except Exception as e:
...
async def knowledge_base_export():
    """导出知识库为 JSON"""
    try:
        from app.knowledge_db import list_all
        data = list_all(limit=100000)
        return {"count": len(data), "data": data}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/knowledge-base/download")
async def knowledge_base_download():
    """下载知识库 SQLite 文件"""
    from fastapi.responses import FileResponse
    from app.knowledge_db import get_db_path
    db_path = get_db_path()
    ...
    return FileResponse(
        ...
        filename="word_knowledge.db",
    )


@router.post("/knowledge-base/sync-from-pg")
async def knowledge_base_sync_from_pg():
    """将 PostgreSQL 中已有的音节/词素数据同步到 SQLite 知识库"""
    try:
        from app.knowledge_db import sync_from_main_db
        count = await sync_from_main_db()
        return {"message": f"已同步 {count} 条记录到知识库", "count": count}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/knowledge-base/apply-to-pg")
async def knowledge_base_apply_to_pg():
    """将 SQLite 知识库数据应用到 PostgreSQL（填充缺失的音节/词素）"""
    try:
        from app.knowledge_db import apply_to_main_db
        count = await apply_to_main_db()
